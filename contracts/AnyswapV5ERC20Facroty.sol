...
contract AnyswapV5ERC20Factory {
    event TokenCreated(address indexed creator, address indexed tagAddress, address indexed tokenAddress, uint toeknIndex);
    ...
    constructor() {
        initCodeHash = keccak256(type(AnyswapV5ERC20).creationCode);
    }
    ...
    function tokenFor(address creator, address tagAddress) external view returns (address token) {
        token = address(uint160(uint256(keccak256(abi.encodePacked(
                hex'ff',
                address(this),
                keccak256(abi.encodePacked(creator, tagAddress)),
                initCodeHash
            )))));
    }

    function createAnyswapERC20ForBridge(
        string memory name, string memory symbol, uint8 decimals,
        address underlying, address mpc, address tagAddress
    ) external returns (address token) {
        token = _createAnyswapERC20(name, symbol, decimals, underlying, mpc, false, tagAddress);
    }

    function createAnyswapERC20ForRouter(
        string memory name, string memory symbol, uint8 decimals,
        address underlying, address router, address tagAddress
    ) external returns (address token) {
        token = _createAnyswapERC20(name, symbol, decimals, underlying, router, true, tagAddress);
    }

    /// @dev create Anyswap ERC20 token.
    /// `tagAddress` is a unique tag to classify a series of tokens.
    /// factory, sender, and tag address could determine the token address.
    function _createAnyswapERC20(
        string memory name, string memory symbol, uint8 decimals,
        address underlying, address vault, bool vaultOnly, address tagAddress
    ) internal returns (address token) {
        bytes memory bytecode = type(AnyswapV5ERC20).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(msg.sender, tagAddress));
        assembly {
            token := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        require(tokenIndex[token] == 0, "token already exist");
        IAnyswapV3ERC20(token).init(name, symbol, decimals, underlying, vault, vaultOnly);
        allTokens.push(token);
        tokenIndex[token] = allTokens.length;

        emit TokenCreated(msg.sender, tagAddress, token, allTokens.length);
    }
}
